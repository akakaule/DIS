...
function Create-ServiceBusForwardDLQSubscription {
    param ([string] $serviceBusNamespace, [string] $topic, [string] $subscription, [string] $dlqforward)
    az servicebus topic subscription create `
    --namespace-name $serviceBusNamespace `
    --topic-name $topic `
    --name $subscription `
    --enable-session $True `
    --forward-dead-lettered-messages-to $dlqforward `
    > $null
    Write-Host "Service Bus Subscription created."
    Write-Host "  Topic: $topic"
    Write-Host "  Subscription: $subscription"
...
# Scope the functions for use inside the parallel loop
	...
	foreach ($endpointtet in $endpoints) {
        ...
        #$endpointId = $_.id
		    $eventsProduced = $endpointtet.EventTypesProduced
        $endpointId = $endpointtet.id
        ...
        # Main subscription
        Create-ServiceBusForwardDLQSubscription     -serviceBusNamespace $sbNamespace -topic $endpointId -subscription $endpointId -dlqforward $resolverId
        ...
        foreach ($eventType in $eventsProduced){
            ...
            foreach ($ce in $consumingEndpoints) {			
                ...
                Create-ServiceBusSubscriptionRule    -serviceBusNamespace $sbNamespace -topic $endpointId -subscription $ce -rule $eventTypeId -filter "user.$nameOfTo = '$eventTypeId'" -action "SET user.$nameOfFrom = '$endpointId'; SET user.EventId = newid(); SET user.To = '$ce';"
            }
        }
